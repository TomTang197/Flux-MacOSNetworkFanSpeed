...
## External Distribution (Developer ID + Notarization)

For distributing to other users, sign with your Developer ID certificate and notarize the package.

### 1. Configure notarization profile (one-time)

```bash
./scripts/setup_notary_profile.sh \
  --profile AEROPULSE_NOTARY \
  --apple-id your-apple-id@example.com \
  --team-id YOUR_TEAM_ID
```

### 2. Build signed release + notarize

```bash
./scripts/release_external.sh --notary-profile AEROPULSE_NOTARY
```
...
If you only want a signed build without notarization:

```bash
./scripts/release_external.sh --no-notarize
```
